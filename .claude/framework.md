...
**Detail and execution playbook live in [`Planning/phase-3-plan.md`](Planning/phase-3-plan.md).** Tomorrow's controller and every dispatched agent reads from there. This section is the high-level summary only — change details there, not here, to avoid divergence.

**Why redesigned.** Phase 2's scaffold pass wrapped every page in an identical `GalleryItemPage { VariantTile { Color.clear } × N }` shell — a uniform-grid container that constrains layout *before* the page knows what it documents. Buttons want condensed grids; sliders want horizontal breathing room; navigation views consume the entire pane; typography wants hand-crafted rows with side-aligned metadata. The OG Typography page (`git show f3a3308:SwiftKit/Pages/Reference/TypographyPage.swift`) proves bespoke per-page layouts work; the redesign generalizes that discipline.

**The fix.** Scaffold and integration merge into one phase. Each page is authored holistically — layout, demos, and per-page reasoning in one pass. Two sub-phases:

- **Phase 3a — Strip (mechanical).** One haiku-tier dispatch clears the 63 SwiftUI placeholder bodies to `EmptyView()`; the `static let item = GalleryItem(...)` extension stays intact so `GalleryRegistry` keeps resolving. Reference + AppKit + scaffold primitives untouched.
- **Phase 3b — Per-page integration.** One opus-tier agent per page, sequential within a folder, parallel across folders within a wave. Per-page chain: layout-design → implementation → spec compliance review → code quality review → logic-log entry. Six waves total: W1 Reference (warm-up) → W2-W5 SwiftUI folders → W6 AppKit re-layout.

**Mandatory baseline skill kit on every Phase 3 dispatch (incl. mechanical 3a):** `superpowers:using-superpowers` + `swiftui-expert-skill`. Task-specific skills (`ui-ux-pro-max`, `find-docs`, `superpowers:brainstorming`, `superpowers:requesting-code-review`, `superpowers:test-driven-development`, `superpowers:verification-before-completion`) layer on top.

**Per-folder Nathan checkpoints** are the only stop conditions: design pre-flight on first page of each folder (Nathan approves the layout pattern), then folder-close visual click-through (dark + light) before the next folder dispatches. Review iteration ceiling: none.

**Per-page logic log** at [`Planning/page-logic-log.md`](Planning/page-logic-log.md) is append-only — each implementer reads the full log before drafting layout, then writes their own entry as their final act. Per-leaf status emojis tracked in [`PlanningTree`](PlanningTree).

---

### Phase 4 — Post-Wave Cleanup, Polish, and Closure

**Detail in [`Planning/phase-3-plan.md`](Planning/phase-3-plan.md) § Phase 4.** Runs after all six Phase 3b waves close AND Nathan signs off on the implementation phase. Pulls together everything deferred during waves: orphan deletions (`PlaceholderGalleryPage`, `DescribePage`, `GalleryPageScaffold`, `ContentView` inline, stale `DetailPane` comment), sidebar + cross-page consistency audit, docs refresh. Per Nathan: open items, orphans, and uncertain components are sorted post-waves — not pre-decided.
...
| Page scaffold primitives | `Detail/PageScaffold/{GalleryItemPage,VariantTile,ReferenceTile,PageSection,APICallout,DemoTiles}.swift` | Locked. Available as a toolkit for Phase 3b page agents. |

**Phase 3-specific composition guidance, OG Typography reference, AppKit pilot disposition, and orphan list live in [`Planning/phase-3-plan.md`](Planning/phase-3-plan.md) — not duplicated here to avoid divergence.**
...
Per Nathan's 2026-05-07 direction: **all Phase 3 open items, orphans, and uncertain components are sorted out in Phase 4 once he signs off on the implementation phase** — not pre-decided here. Phase-3-specific open items live in [`Planning/phase-3-plan.md`](Planning/phase-3-plan.md) § Open Items. The post-MVP-horizon items below remain visible at framework level:
