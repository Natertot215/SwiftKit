---
url: https://developer.apple.com/documentation/swiftui/scene/immersivecontentbrightness(_:)
framework: SwiftUI
...
Pass one of the standard brightness levels defined in `ImmersiveContentBrightness` or a custom one that you create with the `ImmersiveContentBrightness/custom(_:)` method to this scene modifier to set a preference for the content brightness in an `ImmersiveSpace`. The system takes the value that you set into account, but might not be able to honor a specific preference.
