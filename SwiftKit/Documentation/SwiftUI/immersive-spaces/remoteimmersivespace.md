---
url: https://developer.apple.com/documentation/swiftui/remoteimmersivespace
framework: SwiftUI
...
Declare types that conform to the `CompositorContent` protocol to access the environment, declare `@State` variables, and use modifiers inside your remote immersive space.
...
Use the `EnvironmentValues/remoteDeviceIdentifier` environment value to identify the device the scene is running on. This identifier can also be used to initialize an `ARKitSession` associated with the remote device.
