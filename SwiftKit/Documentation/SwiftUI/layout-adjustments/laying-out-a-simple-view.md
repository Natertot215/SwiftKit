...
You can constrain a view to a fixed size by adding a frame modifier. For example, use the `View/frame(width:height:alignment:)` modifier to limit the width the circle to `40` points:
...
If you want the top of the circle aligned with the top of the message content text, you can refine the view by applying an alignment to the `HStack`. To position the content vertically within the stack, specify the `alignment` parameter to `VerticalAlignment/top`:
...
To avoid visually crowding the outer edges of a view, add padding. This introduces a fixed amount of space along the specified edges, reducing the space available for the contents of the view by a corresponding amount. For example, you can use `View/padding(_:_:)` to add extra space along the `Edge/Set/horizontal` edges of the `HStack`:
