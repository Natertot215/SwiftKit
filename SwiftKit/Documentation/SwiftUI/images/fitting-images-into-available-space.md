...
Rendering an image at anything other than its original size requires *interpolation*interpolation: using the existing image data to approximate a representation at a different size. Different approaches to performing interpolation have different trade-offs between computational complexity and visual quality of the rendered image. You can use the `Image/interpolation(_:)` modifier to provide a hint for SwiftUI rendering behavior.
...
Passing the `Image/Interpolation/none` value to `Image/interpolation(_:)` results in a highly pixelated rendered image.
...
When you have an image that’s much smaller than the space you want to render it into, another option  to fill the space is *tiling*tiling: repeating the same image over and over again. To tile an image, pass the `Image/ResizingMode/tile` parameter to the `Image/resizable(capInsets:resizingMode:)` modifier:
