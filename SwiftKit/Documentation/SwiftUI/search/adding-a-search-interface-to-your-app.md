...
Add a search interface to your app by applying one of the searchable view modifiers — like `View/searchable(text:placement:prompt:)` — to a `NavigationSplitView` or `NavigationStack`, or to a view inside one of these. A search field then appears in the toolbar. The precise placement and appearance of the search field depends on the platform, where you put the modifier in code, and its configuration.
...
You can automatically place the search field by adding the `View/searchable(text:placement:prompt:)` modifier to a navigation element like a navigation split view:
...
You can alternatively use the `placement` input parameter to suggest a `SearchFieldPlacement` value for the search interface. For example, you can achieve the same results as the previous example in macOS using the `SearchFieldPlacement/sidebar` placement:
