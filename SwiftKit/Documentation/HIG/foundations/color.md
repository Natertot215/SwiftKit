...
**Test your app on different devices.** For example, the True Tone display — available on certain iPhone, iPad, and Mac models — uses ambient light sensors to automatically adjust the white point of the display to adapt to the lighting conditions of the current environment. Apps that primarily support reading, photos, video, and gaming can strengthen or weaken this effect by specifying a white point adaptivity style. Test tvOS apps on multiple brands of HD and 4K TVs, and with different display settings. You can also test the appearance of your app using different color profiles on a Mac — such as P3 and Standard RGB (sRGB) — by choosing a profile in System Settings > Displays.
...
iOS defines two sets of dynamic background colors — *system* and *grouped* — each of which contains primary, secondary, and tertiary variants that help you convey a hierarchy of information. In general, use the grouped background colors when you have a grouped table view; otherwise, use the system set of background colors.
...
|-------|----------|-----------|
| Label | A text label that contains primary content. | `UIColor.label` |
| Secondary label | A text label that contains secondary content. | `UIColor.secondaryLabel` |
| Tertiary label | A text label that contains tertiary content. | `UIColor.tertiaryLabel` |
| Quaternary label | A text label that contains quaternary content. | `UIColor.quaternaryLabel` |
| Placeholder text | Placeholder text in controls or text views. | `UIColor.placeholderText` |
| Separator | A separator that allows some underlying content to be visible. | `UIColor.separator` |
| Opaque separator | A separator that doesn't allow any underlying content to be visible. | `UIColor.opaqueSeparator` |
| Link | Text that functions as a link. | `UIColor.link` |

### macOS

macOS defines extensive dynamic system colors including:
...
**Recognize that people might prefer graphic complications to use tinted mode instead of full color.** The system can use a single color that's based on the wearer's selected color in a graphic complication's images, gauges, and text.

## Specifications

### System colors

The specification section includes detailed color swatches for:
- Red, Orange, Yellow, Green, Mint, Teal, Cyan, Blue, Indigo, Purple, Pink, and Brown
- Each with variants for: Default (light), Default (dark), Increased contrast (light), and Increased contrast (dark)

*visionOS system colors use the default dark color values.*

### iOS, iPadOS system gray colors

Includes Gray, Gray (2) through Gray (6) with variants for light, dark, and increased contrast modes.
