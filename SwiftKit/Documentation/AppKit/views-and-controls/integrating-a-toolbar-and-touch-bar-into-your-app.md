...
The *toolbar*toolbar appears in the space immediately below or next to a window’s title bar and above the app’s content. You use an `NSToolbar` object to manage the items that appear in a toolbar, which are `NSToolbarItem` objects created in Interface Builder or source code.
...
To tell the toolbar which items are available, the sample’s toolbar delegate implements the `NSToolbarDelegate/toolbarAllowedItemIdentifiers(_:)` method, which returns an array of item identifiers. The available items appear in the toolbar’s customization palette for customizing the toolbar when running the sample app.
...
When the sample app launches for the first time, a default set of items appear in the toolbar. The sample provides these items by implementing the `NSToolbarDelegate/toolbarDefaultItemIdentifiers(_:)` delegate method, which returns an array containing the font style and font size item identifiers.
...
The toolbar asks its delegate to create a toolbar item by calling the `NSToolbarDelegate/toolbar(_:itemForItemIdentifier:willBeInsertedIntoToolbar:)` method. It calls this method when adding an item to the toolbar but also when adding items to the toolbar’s customization palette.
...
When the toolbar calls the delegate method `NSToolbarDelegate/toolbar(_:itemForItemIdentifier:willBeInsertedIntoToolbar:)`, the sample creates the toolbar item by calling its helper function `customToolbarItem`, passing in the custom view for the requested toolbar item.
...
The sample’s toolbar delegate also implements the `NSToolbarDelegate/toolbarWillAddItem(_:)` method to know when the toolbar is about to add an item. This gives the delegate the opportunity to add or change state information for the item. For example, the sample sets the `NSToolbarItem/toolTip` property of a toolbar item with the `NSToolbarItem/Identifier/print` identifier.
...
The toolbar displays the default items in the same order as they appear in the array that `NSToolbarDelegate/toolbarDefaultItemIdentifiers(_:)` returns. But users can rearrange the items, add and remove items, and reset the toolbar to its default items by selecting View > Customize Toolbar, which displays the toolbar’s configuration palette. The sample app also saves the changes and reapplies them the next time the user launches the app.

The sample app provides these behaviors by setting the toolbar’s `NSToolbar/allowsUserCustomization` and `NSToolbar/autosavesConfiguration` properties to `true`.
...
Before the sample app can add items to the Touch Bar, it needs to create an `NSTouchBar` object. The sample does this by overriding the `NSResponder/makeTouchBar()` method, where it creates and returns a new Touch Bar object.

Then, similar to adding toolbar items, the sample implements the `NSTouchBarDelegate` method `NSTouchBarDelegate/touchBar(_:makeItemForIdentifier:)`, where it creates and returns Touch Bar items for the specified identifier.
...
As with the toolbar, the sample app lets users customize items in the Touch Bar by choosing View > Customize Touch Bar. The app provides this feature by setting the `NSApplication/isAutomaticCustomizeTouchBarMenuItemEnabled` property to `true`.
