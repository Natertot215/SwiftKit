...
Apple provides several Sign in with Apple buttons you can use to let people set up an account and sign in. If necessary, you can create a custom button to offer Sign in with Apple; for guidelines, see `sign-in-with-apple#Creating-a-custom-Sign-in-with-Apple-button`.
...
For developer guidance, see `ASAuthorizationAppleIDButton` (iOS, macOS, and tvOS), `WKInterfaceAuthorizationAppleIDButton` (watchOS), and `displaying-sign-in-with-apple-buttons-on-the-web`. You can also visit `button` to view and adjust live previews of web-based buttons and get the code.
...
Use only the logo artwork downloaded from ``; never create a custom Apple logo. As you create a custom Sign in with Apple button, follow these guidelines for using the downloadable logo file:
