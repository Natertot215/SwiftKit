...
A good place to do this is in your app delegate’s `NSApplicationDelegate/applicationWillFinishLaunching(_:)` method as the following example shows:
...
For example, the following code demonstrates how to display a contextual menu in response to a mouse-down event, and have AppKit insert the menu item. This code overrides the `NSResponder/mouseDown(with:)` method and creates a menu. It then invokes the `NSMenu` class method `NSMenu/popUpContextMenu(_:with:for:)`, passing the event object related to the mouse-down event and the view that owns the contextual menu. AppKit automatically inserts the Continuity Camera menu item in the contextual menu for you.
