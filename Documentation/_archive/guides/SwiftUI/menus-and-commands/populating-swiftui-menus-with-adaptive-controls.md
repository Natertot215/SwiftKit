...
> **NOTE:** SwiftUI controls and views are adaptive, and represent functionality and meaning, as well as visual representation. When you open a menu, the menu items appear in a context-appropriate order depending on the platform. For more information, see `View/menuOrder(_:)`.

To show a symbol next to the menu item title, use the `Button/init(_:systemImage:action:)` initializer:
...
Add a visual warning cue to menu items that are destructive by nature. Add a `ButtonRole/destructive` role to `Button` to tint the menu item red. Use `destructive` only for actions that require caution.
...
On macOS, menu items constructed with a `Label` render without an icon by default. Use the `LabelStyle/titleAndIcon` style to override the system behavior and explicitly render an icon for the menu items.
...
You can choose picker styles such as `PickerStyle/inline`, `PickerStyle/menu`, and `PickerStyle/palette`.
...
By default, menus dismiss as soon as someone clicks or taps an item. If you want the person to make multiple selections, or repeat an action without reopening the menu, override this behavior with the `View/menuActionDismissBehavior(_:)` modifier on specific items.
