---
url: https://developer.apple.com/documentation/appkit/nsignoremisspelledwords
framework: AppKit
...
The `NSIgnoreMisspelledWords` protocol specifies a  method, `NSIgnoreMisspelledWords/ignoreSpelling(_:)`, which should be implemented like this:
...
The second argument to the `NSSpellChecker` method `NSSpellChecker/ignoreWord(_:inSpellDocumentWithTag:)` is a tag that the spell checker can use to distinguish the documents being checked. When the spell checker can distinguish the various documents, it can append new ignored words to the appropriate list.
...
**Conforming Types**: `NSText`, `NSTextView`
