...
- `ThumbnailAction` operates on *any*any kind of image.
...
When a user invokes an Action Extension, the system instantiates the `https://developer.apple.com/library/archive/documentation/General/Reference/InfoPlistKeyReference/Articles/AppExtensionKeys.html#//apple_ref/doc/uid/TP40014212-SW3` specified in the `Info.plist` and asks it to handle the request through the `NSExtensionRequestHandling` protocol.

The system calls the `NSExtensionRequestHandling/beginRequest(with:)` method and passes in an `NSExtensionContext`. To access the input files, use the `NSExtensionItem/attachments` property of the `NSExtensionItem` contained in the `inputItems` array.

Once processing completes, pass output files back to the system by creating new `NSItemProvider` objects and registering them with `NSItemProvider/registerFileRepresentation(forTypeIdentifier:fileOptions:visibility:loadHandler:)`.
...
To obtain the correct directory path for writing output files, use `FileManager/url(for:in:appropriateFor:create:)`, passing the `FileManager/SearchPathDirectory/itemReplacementDirectory` constant.
...
Finally, signal that the action is complete by calling `NSExtensionContext/completeRequest(returningItems:completionHandler:)`.
...
The `UppercaseAction` extension accepts any text, either supplied as attachments in the same way as `ThumbnailAction` and `RemoveOpacityAction`, or from other sources such as a standard `NSTextField` control. Check the `NSExtensionItem/attributedContentText` property for input from text fields.

If `attributedContentText` is empty, check for input in the `NSExtensionItem/attachments` property of `NSExtensionItem`.
...
> **NOTE:** When creating a new Action Extension target, Xcode prompts you to decide if your extension will present a user interface. Based on that choice, Xcode sets appropriate initial values for `NSExtensionPointIdentifier` and `https://developer.apple.com/library/archive/documentation/General/Reference/InfoPlistKeyReference/Articles/AppExtensionKeys.html#//apple_ref/doc/uid/TP40014212-SW3`.

Extensions that present a user interface show their view controller’s view immediately after the extension calls `NSExtensionRequestHandling/beginRequest(with:)` and should input files with the `NSViewController/extensionContext` property of `NSViewController`.

Once the extension’s user interface has completed any required user interaction for the task, process the files in the normal way by calling `NSExtensionContext/completeRequest(returningItems:completionHandler:)`.

To cancel the task, use the `NSExtensionContext/cancelRequest(withError:)`. Pass a `NSUserCancelledError-swift.var` if the user canceled the task, or an error caused the failure.
