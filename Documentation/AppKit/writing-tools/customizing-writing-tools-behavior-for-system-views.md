---
url: https://developer.apple.com/documentation/appkit/customizing-writing-tools-behavior-for-system-views
framework: AppKit
...
> **NOTE:** If you create a custom text view, instead of using the standard system views, use the available APIs to add Writing Tools support to your view. For more information, see `adding-writing-tools-support-to-a-custom-nsview`.
...
In a `NSTextView` you can determine when Writing Tools is active using your delegate object. The system calls the delegate’s `NSTextViewDelegate/textViewWritingToolsWillBegin(_:)` method before Writing Tools makes any modifications, and it calls the `NSTextViewDelegate/textViewWritingToolsDidEnd(_:)` method after incorporating any final changes. Use those methods to turn off any features that might interfere with Writing Tools while it’s active.

#### Specify ranges of text for Writing Tools to ignore

If you don’t want Writing Tools to modify portions of your `NSTextView` object, implement the `NSTextViewDelegate/textView(_:writingToolsIgnoredRangesInEnclosingRange:)` method in your delegate. Writing Tools calls that method for each operation, providing you with the range of text it’s considering. Use your implementation of the method to specify any subranges of text you want Writing Tools to ignore. You might use this method to prevent Writing Tools from changing code listings, proper names, or direct quotes embedded in the text.
