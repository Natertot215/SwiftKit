---
url: https://developer.apple.com/documentation/swiftui/building-and-customizing-the-menu-bar-with-swiftui
framework: SwiftUI
category: Scenes
...
Custom menu items are created with standard SwiftUI views, for example `Button` and `Toggle`. `Menu` creates a submenu.

The menu bar also displays information about keyboard shortcuts next to menu items and which keys someone needs to press on the keyboard to perform an action without using the menu bar. The `keyboardShortcut(_:)` modifier allows you to define which key combination will perform the action.
...
Use `FocusedValue` to create contextual dependencies with your menus and menu items. A focused value is state data that requires an active scene with its view hierarchy in focus. Use a dynamic property to react to changes in the views of the scene.
...
Use the `FocusedValue` property wrapper to represent the active scene's data model in the menu bar:
...
### Using Value Types with FocusedValues Entry
