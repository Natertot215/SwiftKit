...
- Find an open scene that indicates it prefers to or can handle the event, if any, and send the event to that scene. You use the `View/handlesExternalEvents(preferring:allowing:)` view modifier on a view inside the scene to register this preference.
...
Make sure that at least one of these rules succeeds in your app for all events that your app claims to handle. Also, make sure that the scene that receives an event actually handles it. For example, be sure that a scene that receives user activities handles them with an appropriate `View/onContinueUserActivity(_:perform:)` view modifier.

Don’t confuse the `handlesExternalEvents(matching:)` scene modifier with the `View/handlesExternalEvents(preferring:allowing:)` *view*view modifier. You use the scene modifier to help SwiftUI choose a new scene to open when no open scene handles an external event, whereas you use the view modifier to indicate that an open scene can or prefers to handle certain events.
...
- For an `NSUserActivity`, like when your app handles Handoff, SwiftUI uses the activity’s `NSUserActivity/targetContentIdentifier` property, or if that’s `nil`, its `NSUserActivity/webpageURL` property rendered as an `URL/absoluteString`.
- For a `URL`, like when another process opens a URL that your app handles, SwiftUI uses the URL’s `URL/absoluteString`.
...
> **IMPORTANT:** `DocumentGroup` scenes ignore this modifier. Instead, document scenes decide whether to open a new scene to handle an external event by comparing the incoming URL or user activity’s `NSUserActivity/webpageURL` against the document group’s supported types.
