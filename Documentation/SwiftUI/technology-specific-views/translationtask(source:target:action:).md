...
To perform new translations with the same `source` and `target` language, use `View/translationTask(_:action:)` and call `TranslationSession/Configuration/invalidate()`.
