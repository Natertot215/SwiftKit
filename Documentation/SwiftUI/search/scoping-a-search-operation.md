...
To use the scope information, bind the current scope to the `View/searchScopes(_:scopes:)` modifier. You also indicate a set of views that correspond to the different scopes. Like the `View/searchSuggestions(_:)` modifier, the scopes modifier operates on the searchable modifier that’s closer to the modified view, so it needs to follow the searchable modifier:
...
You can change when the picker appears by using the `View/searchScopes(_:activation:_:)` modifier instead, and supplying one of the `SearchScopeActivation` values, like `SearchScopeActivation/onTextEntry` or `SearchScopeActivation/onSearchPresentation`.
